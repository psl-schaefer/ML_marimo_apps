...
def _(
    ...
    X, Y, Z = mus_tested_1.reshape((mus_tested.shape[0], mus_tested.shape[0])).numpy(), mus_tested_2.reshape((mus_tested.shape[0], mus_tested.shape[0])).numpy(), F_probs.reshape((mus_tested.shape[0], mus_tested.shape[0]))
    ax[2].contour(X, Y, Z, levels=20, alpha=0.5)

    # also plot the Eigen vectors of empirical F
...
def _(
    ...
    fig_2, ax_2 = plt.subplots(2, 3, figsize=(15, 10))
    ax_2 = ax_2.flatten()

    selected_samples = torch.tensor([[2.5, 7.5], [2.5, 2.5], [7.5, 2.5], [7.5, 7.5], [5., 5.], [2.5, 5.]])

    selected_log_probs = torch.cat([dist.MultivariateNormal(torch.tensor([mus_tested_1[i].item(), mus_tested_2[i].item()]), sigma).log_prob(selected_samples) 
                           for i in range(len(mus_tested_1))]).reshape(-1, len(selected_samples)).numpy()

    selected_grads_at_true = log_lik_grad(x=selected_samples, mu=true_mu, precision=prec).numpy()

    for plot_idx in range(len(selected_samples)):
        XX, YY, ZZ = mus_tested_1.reshape((mus_tested.shape[0], mus_tested.shape[0])).numpy(), mus_tested_2.reshape((mus_tested.shape[0], mus_tested.shape[0])).numpy(), selected_log_probs[:, plot_idx].reshape((mus_tested.shape[0], mus_tested.shape[0]))
        ax_2[plot_idx].contour(XX, YY, ZZ, levels=np.linspace(-100, 0, num=30), alpha=0.5, cmap="viridis")
        ax_2[plot_idx].axvline(true_mu[0], color='white', linestyle='--', alpha=0.3)
        ax_2[plot_idx].axhline(true_mu[1], color='white', linestyle='--', alpha=0.3)

    for plot_idx in range(len(selected_samples)):
        ax_2[plot_idx].set_xlabel("$\mu_1$")
        ax_2[plot_idx].set_ylabel("$\mu_2$")

    for plot_idx in range(len(selected_samples)):
        ax_2[plot_idx].scatter(selected_samples[plot_idx, 0], selected_samples[plot_idx, 1], color='blue', s=100, alpha=0.4)

    for plot_idx in range(len(selected_samples)):
        ax_2[plot_idx].quiver(np.repeat(true_mu[0], len(selected_samples)), np.repeat(true_mu[1], len(selected_samples)), selected_grads_at_true[plot_idx, 0], selected_grads_at_true[plot_idx, 1], 
              angles='xy', scale_units='xy', scale=0.5, color='white')

    fig_2.suptitle(f"Contours: Log Likelihood, i.e. $\log p(x_i | \mu, \sigma)$\nVectors: Scores (Gradient of Log Likelihood), i.e. $\\nabla_x \log p(x_i | \mu, \sigma)$ evaluated at $\mu^*$")
